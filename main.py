...
=====================
FastAPI backend for the NLP-powered content-based movie recommendation engine.
Loads a pre-trained cosine similarity matrix and movie metadata from pickle files,
and serves recommendations via a REST endpoint.
...
# ── Lifespan: load model files once at startup ────────────────────────────────
@asynccontextmanager
...
def fetch_movie_details(movie_id: int) -> dict:
    """Fetch poster, rating, release year, genres and overview from TMDB."""
    try:
...
async def list_movies():
    """Return all movie titles for the frontend dropdown."""
...
async def recommend(movie: str, n: int = 5):
    """
    Return top-N content-based recommendations for the given movie title.

    Parameters
    - **movie**: exact movie title (use /movies to get valid titles)
    - **n**: number of recommendations (1–10, default 5)
    """
